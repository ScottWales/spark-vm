...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sudo yum install -y wget java-1.7.0-openjdk centos-release-scl
    sudo yum install -y python27
    sudo scl enable python27 -- pip install --upgrade jupyter

    curl http://d3kbcqa49mib13.cloudfront.net/spark-2.0.0-bin-hadoop2.7.tgz -o spark.tar.gz
    sudo mkdir -p /opt/spark
    sudo tar -C /opt/spark --strip-components 1 -xf spark.tar.gz
